...
`tools/list` at `https://gateway.pipeworx.io/lorem/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/generate_paragraphs \
  -H 'Content-Type: application/json' \
  -d '{"count":3,"length":"medium"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/generate_paragraphs`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
